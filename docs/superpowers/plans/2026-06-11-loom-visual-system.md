...
**Goal:** Rebuild the whole Loom visual system to the cool-black "Evidence Desk" design — luminance-layered structure, signature cyan, real optical glass, four scene light-environments — across every surface, keeping the contract suite green.
...
**Spec:** `docs/superpowers/specs/2026-06-11-loom-visual-system-design.md` — read it; this plan implements it exactly. The banned anti-patterns (solid accent fills, saturated discs, fake glass) are hard constraints.
...
- [ ] **Step 1:** Add the foundation custom properties to `:root` (cold graphite-black with a restrained silver lift — NOT flat black, NOT pale silver, NOT black-blue):
```css
:root{
  --ink-0:#070809; --ink-1:#0B0C0D; --ink-2:#111315; --ink-3:#181B1E;
  --ink-4:#22262A; --ink-5:#30353A; --line:#2A2F34; --line-soft:#202428;
  --text-1:#E7E9EA; --text-2:#A4A9AD; --text-3:#666D72;
  --accent:#4BC5DE; --accent-secondary:#8AF7E6; --gold:#4BC5DE; --gold-hi:#8AF7E6; --cyan:#6CE7F2; --up:#3FB37A; --down:#E06A6A;
  --serif:'Cormorant Garamond',Georgia,serif; --mono:ui-monospace,'JetBrains Mono',Menlo,monospace;
  /* key light, top-left */
  --keylight: inset 0 1px 0 rgba(255,255,255,0.07);
...
```css
.loom-glass{ background:linear-gradient(145deg,rgba(255,255,255,0.065),rgba(255,255,255,0.018) 38%,transparent 72%),rgba(18,20,22,0.58); -webkit-backdrop-filter:blur(30px) saturate(118%) brightness(1.05); backdrop-filter:blur(30px) saturate(118%) brightness(1.05);
  border:1px solid var(--line); border-radius:14px;
  box-shadow: var(--keylight), inset 0 -1px 0 rgba(0,0,0,0.38), var(--float-2); }
.loom-material{ background:linear-gradient(150deg,rgba(255,255,255,0.055),transparent 30%),linear-gradient(180deg,var(--ink-4),var(--ink-3)); border:1px solid var(--line); box-shadow:var(--keylight),var(--float-1); }
.loom-gold-mark{ color:var(--gold); } /* legacy name; resolves to signature cyan; text/stroke only — never a fill */
...
- [ ] **Step 4:** Update theme tests: grep each of the three test files for the old hex tokens (`#C8A24A` / `#E3C56A` / `#FCD535` / `#C9A15C` / `#9E7C3E` / `#0B0B0B` etc.) and re-point colour/theme assertions to the signature-cyan tokens. Keep structural assertions.
...
- [ ] **Step 1:** Re-point `.lcv` vars to the foundation: base `--ink-0`, max whitespace, accent budget = signature cyan ~2 touches (active nav underline + one numeral mark), data cyan only in real data artifacts. Replace every literal champagne/#FCD535/#C8A24A-as-fill with token references; ensure NO accent fill anywhere (active nav = 1px signature underline only; row numbers = `--text-2` mono, not accent pills).
...
- [ ] **Step 3:** Screenshot `http://localhost:3000/` at 1488 + 1920; verify against spec §3 Cover row (deepest black, ~2 signature-cyan touches, real artifacts, zero fills). Fix deviations.
...
- [ ] **Step 1:** Re-point `.vd-section-page` vars: base `--ink-2` (lifted), neutral, material strength DOWN, signature cyan only on verified marks (stroke, not fill). Documents/evidence read clearly. Keep the unified nav (1px signature active underline).
...
- [ ] **Step 1:** Re-point `--role-*` to the foundation: base `--ink-1` + faint cool ambient glow; data cyan `--cyan` for data-signal (active capability indicator, the Sources->Draft->Answer flow line as a thread-of-light, "thinking" state); signature cyan ONLY on the active tab/row as a 1px left-border or underline — NOT a filled tab. Kill the yellow-fill active states.
...
- [ ] **Step 3:** Screenshot `/digital-me` (1488); verify signal/alive, data-cyan-on-data, signature cyan sparse. `npm run test:contracts` (digital-me-role-os) + typecheck; commit `style(loom): Digital Me scene — signal`.
...
- [ ] **Step 2:** Colour only on data: order book + PnL + ticker use `--up`/`--down` (small), price-graph sparklines + chart lines use `--cyan`; signature cyan only on the active control. Structure stays achromatic.
...
### Task 7: App icon — signature-cyan re-cut

**Files:** Modify `public/icon.svg`; regenerate via the icon script.

- [ ] **Step 1:** Re-cut the icon to the discipline: the flat bold yellow "L" is a toy. New: a finer **signature-cyan hairline** woven/lunar mark, more cool-black luminance/depth in the tile (a subtle top key-light sheen), and the bright data-cyan weft as a single thin thread. Accent area stays minimal. Keep it legible at 32px.
...
- [ ] **Step 3:** `npx tsx --test tests/app-store-assets.test.ts` (pass); view the 512 + 32 renders; iterate once if toy-ish. Commit `style(loom): app icon re-cut to signature-cyan discipline`.

---

### Task 8: Refresh README screenshots + deck

**Files:** Re-capture `docs/images/product/*.png` from the new pages; regenerate `docs/deck/loom.pptx` (palette -> cool-black/signature-cyan per `docs/deck/build-loom-deck.mjs`).
...
- [ ] **Step 2:** Update `docs/deck/build-loom-deck.mjs` palette to the foundation tokens (cool-black, signature cyan, data cyan), rebuild `loom.pptx`. Update README copy where it says Optibook->QBook.
...
- [ ] **Step 2:** Screenshot every route at 1488 + 1920; judge each against spec §3 (right scene mood, signature color <5% & never filled, no saturated discs, real glass/material/light, no cream/champagne leftovers). Use a fresh subagent for visual QA (fresh eyes).
