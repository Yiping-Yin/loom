...
This file preserves the May acceptance evidence, but its `Sources / Draft`
product definition has been superseded by the 2026-06-27 contract:

- Current user-facing IA is `Sources / Studio / Digital Me`.
- `Sources` owns collection and context resolution.
- `Studio` owns the block document/form workbench. `Draft` remains a route,
  storage, prompt, and test compatibility name for this engine.
- `Digital Me` owns selected source-backed representation and cited answers.
- `/draft` remains a compatibility entry into Studio, not a primary product
  destination.
...
- `docs/loom.md` now opens with a current operating definition for Sources /
  Studio / Digital Me and marks older `Sources / Draft`, `Collect / Organize /
  Draft`, weaving, and substrate language as historical or compatibility
  context, not feature naming guidance.
...
- Studio owns the writing/form surface and an action-first Details layer with
  separate source, edit, provenance, and board controls. Draft remains the
  implementation compatibility name.
- Draft/Studio provider prompts now bound title, body, selected text, attached
  references, inline references, and corpus context before sending, for both
  Continue writing and Cmd-K inline edit, so multi-source Studio documents do
  not silently exceed small local provider context windows.
...
- Capture artifact states now flow from native CaptureAST sidecars through
  `captures-list.json`, Sources-to-Studio handoff links, native
  capture-to-Draft compatibility attach, and Studio/Draft/Compile prompt
  reference metadata, so structured diagrams such as the flipdisc frame format
  are not reduced to flat token snippets before provider review.
- Sources owns file intake, recent captures, source groups, reading/review queues, and Studio queues.
...
- Keyboard Help uses literal source/studio actions and no longer advertises old thought-anchor, warp-thread, weave, or panel-memory language.
...
- Default-visible product copy uses literal source/studio/digital-me language across metadata, support/privacy pages, shortcuts, source review buttons, first-run copy, and reader-note surfaces.
- The web root layout no longer mounts retired Rehearsal, Examiner, or Reconstructions overlays; revision and refresh actions route to Studio or Sources reader notes.
...
- About page copy should be grounded in the Sources / Studio / Digital Me loop instead of Vellum-era panel/weave brand language.
- App Store copy and screenshot defaults should use product surfaces (`Sources`, `Add source`, `Studio`, `Digital Me`, `Reader notes`, `Connections`) instead of `collect` wording or the old `/frontispiece` identity page.
...
- Knowledge-data docs should define current Sources / Studio / Digital Me data responsibilities,
...
- `docs/projects/active/README.md` now defines the continuation reading order
  for new Loom work: product definition and remake audit first, then the older
  acceptance status, completion audit, and legacy-surface migration map as
  historical gate evidence. The older 2026-05-08 skeleton docs are historical
  reference only and old Phase 1 / `Collect` / `Organize` / `Draft` snippets
  must be translated into the current `Sources` / `Studio` / `Digital Me` model
  before acting.
...
- `npx tsx --test tests/new-loom-skeleton-contract.test.ts --test-name-pattern "default-visible product copy uses literal Sources Studio and Digital Me vocabulary"`
...
5. Re-run installed-app smoke and the focused chrome verifier.
   - Expected evidence: installed app smoke passes and Sources/Studio/Digital Me chrome remains compact and non-overlapping, with `/draft` verified only as a compatibility path.
